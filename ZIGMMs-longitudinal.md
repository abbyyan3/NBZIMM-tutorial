...
for (i in 1:n.sims){
  tryCatch({
    ...
    f5 = lme.zig(y2 ~ x|1 , zi.random = F, random = ~ 1 | ind, verbose = F) 
    out3[i, ] = summary(f5)$tTable[2, c(1,2,5)]
...
for (j in 1:ncol(yy)){
  tryCatch({
    ...
    f5 = lme.zig(y1 ~ group + weeks  + age + race + offset(log(N)), random = ~ 1|subject)
    out2[j, ] = summary(f5)$tTable[2, 5]
    
    f6 = lme.zig(y1 ~ group + weeks  + age + race + offset(log(N)), family = "zig", random = list(subject = pdDiag(~weeks)))
    out4[j, ] = summary(f6)$tTable[2, 5]
    
    f7 = lme.zig(y1 ~ group*weeks  + age + race + offset(log(N)), family = "zig", random = ~ 1|subject)
    out6[j, ] = summary(f7)$tTable[2, 5]
    out8[j, ] = summary(f7)$tTable[6, 5]
    
    f8 = lme.zig(y1 ~ group*weeks  + age + race + offset(log(N)), family = "zig", random = list(subject = pdDiag(~weeks)))
